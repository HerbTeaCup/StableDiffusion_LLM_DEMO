...
public class ModelDropDown : BaseDropDown<SDmodel>
{
    ...
    public override async void OnValueChanged(int index)
    {
        ...
        await Communication.PostRequestAsync<Config>(urlManager.StableDiffusion.GetUrl(StableDiffusionRequestPurpose.Options), ManagerResister.GetManager<SDManager>().config);//WebUI Config와 같게(일관성)
